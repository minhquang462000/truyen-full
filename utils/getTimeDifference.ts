export function getTimeDifference(dateString: string) {
    ...
    const result: { days?: number; hours?: number; minutes?: number; seconds: number } = { seconds };
...
export const timeFormat = (time: string) => formatTimeDifference(getTimeDifference(time));
