...
export default function ChangeListByRat({ books }: { books: IBook[] }) {
  const [activeRat, setActiveRat] = useState<string>("day");
  const [bookData, setBookData] = useState<IBook[]>(books);
  useEffect(() => {
    const fetchDataBookByRat = async () => {
      try {
        const res = await axios.get(
          `${DOMAIN}/api/client/books?sortKey=${activeRat}`
        );
  ...
  return (
    <div className="w-full text-sm ">
      ...
      <div className="flex w-full mt-4 text-[12px] font-semibold justify-center items-center gap-2">
        <button
          onClick={() => setActiveRat("day")}
          className={`border min-w-[60px] px-4  py-[6px] border-[#aaa]  rounded-full hover:bg-[#4e4e4e] hover:text-white ${activeRat === "day"
        ...
        <button
          onClick={() => setActiveRat("views")}
          className={`border min-w-[60px] px-4  py-[6px] border-[#aaa]  rounded-full hover:bg-[#4e4e4e]  hover:text-white ${activeRat === "views"
            ? "bg-[#4e4e4e] border-[#4e4e4e] text-white"
            : "text-[#4e4e4e] "
            }`}
        >
