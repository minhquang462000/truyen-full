...
export default function ListStoryByAuthor({ books }: { books: IBook[] }) {
    return (
        <div className="bg-[#ecf0f1] border-[1px] dark:border-transparent dark:bg-transparent mb-8 border-[#D9E1E4] p-2 ">
            <div className="w-full relative  dark: border-transparent border-b border-[#ccc] flex items-center font-medium justify-between">
                <h2 className="flex gap-1 dark:border-transparent dark:text-white  text-lg pl-1 border-b border-[#333] md:pl-0 pb-1 items-center">
                    <Link href=""> {"Truyện cùng tác giả".toUpperCase()}</Link>
                </h2>
            </div>
            <ul className="w-full mt-3 text-sm flex flex-col">
                {books?.map((book, index) => (
                    <li onClick={() => { handleUpdateView(book?._id) }} key={index}
                        className="flex dark:border-transparent dark:bg-[#292929]  items-center py-2 border-b border-dashed border-[#ccc]  hover:underline">
